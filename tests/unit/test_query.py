...
def test_search_keyed(query_handler):
...
def test_search_sources(query_handler):
    ...
    resp = query_handler.search("cisplatin", keyed=True, excl=sources)
...
def test_infer_option(query_handler):
    ...
    query = "DB01174"
    response = query_handler.normalize(query, infer=False)
    assert response.query == query
...
def test_query_normalize(query_handler, normalized_phenobarbital,
...
def test_unmerged_normalize(query_handler, compare_records,
...
def test_merged_meta(query_handler):
